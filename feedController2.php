...
class feedController
{
	...
	public function opinionSubmit($questionId,$opinion)
	{
		global $pdo;
		global $user_id;
		$opinion = htmlspecialchars(htmlentities($opinion));
			...
			echo'</div>
				...
				echo'</div>
					<div class="comment-text">
						'.htmlspecialchars($opinion->opinion).'
					</div>
					...
					{
?>
						<i class="fa fa-thumbs-up <?php if($checkLikeFlag==1){echo 'like';}?>" id="likeBtn<?php echo $opinion->id;?>" onclick="checkLike(<?php echo $opinion->id;?>)"></i>&nbsp;<span id="like<?php echo $opinion->id;?>"><?php echo $opinion->likes;?></span>
<?php
					}
					else
					{
		...
		foreach ($data as $item) 
		{
			echo'<div class="post-container" id="'.$item->id.'">
					<div class="post-question" >
						<b><a href="question.php?qid='.$item->id.'">'.htmlspecialchars($item->question).'</a></b>
					</div>';
...
<?php
				echo'<div class="post-details">
						...
						<div class="comment-count"><i class="fa fa-comment-o"></i> : <span id="opinion'.$item->id.'">'.$item->opinion.'</span></div>
						<div id="menu'.$item->id.'" class="triple-dot-menu-container" tabindex="-1">
...
?>
								...
								<div class="twitter-share-question"><i class="fa fa-twitter"></i>Share on Twitter</div>
								<div class="report-question"><i class="fa fa-ban"></i>Report</div>
<?php
						echo'</div>
						</div>
					</div>';
				echo'<div class="question-comments-container">
						<form id="opinionForm'.$item->id.'" method="post">
							<textarea name="opinion" placeholder="Write your opinion.." rows="4"></textarea>
							<input type="hidden" name="question_id" value="'.$item->id.'">
							<div class="comment-post-button">
								<input type="submit" id="opinionBtn'.$item->id.'" class="btn btn-primary" onclick="opinionSubmit('.$item->id.')" value="Opinion">
							</div>
						</form>
						<span class="opinionView'.$item->id.'"></span>
					</div>
				</div>';
	...
	<script type="text/javascript">
		...
		function opinionView(questionId)
		{
				$.ajax({
					...
					beforeSend:function()
					{
						$(".opinionView"+questionId).html('<div class="loader"><div></div></div>');
					},
					success: function(data)
					{
						$(".opinionView"+questionId).slideDown();
		...
		{
			$("#opinionForm"+questionId).on("submit",function(e){
				e.preventDefault();
